 ...
 *  - Session cookie is `HttpOnly; SameSite=Strict; Secure`. No CSRF token needed.
...
export function createPortalHandler(deps: PortalDeps): ApiHandler {
  ...
  function sessionCookie(token: string): string {
    return `${COOKIE}=${token}; HttpOnly; SameSite=Strict; Path=/portal; Secure`;
  }

  function clearCookie(): string {
    return `${COOKIE}=; HttpOnly; SameSite=Strict; Path=/portal; Secure; Max-Age=0`;
  }
