# Archived ledger — 2026-05 (legacy `LOOP_LOG.md`)

> **Provenance:** This is the pre-migration operational ledger, preserved verbatim under
> Axiom 3 (*Preserve Every Attempt*). The 2026-05-24 substrate refresh (`1159d45`) installed a
> new canonical logging system at `docs/LOG.md` with a strict, machine-validated schema and
> forbade a separate `LOOP_LOG.md` from existing in the workspace. Iterations 1–75 were moved
> here intact during iter-0076 rather than rewritten. **New entries live in `docs/LOG.md`**;
> this file is read-only history (legacy `## YYYY-MM-DD — Iteration N` schema below).
