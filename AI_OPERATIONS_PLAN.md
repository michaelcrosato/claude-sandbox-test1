# AI Operations Plan — claude-sandbox-test1 (Posthorn)
...
| App hosting / QA surface | **Docker / Fly.io** — single-container image built on each PR; persistent staging deployment from `develop` on Fly.io | A non-technical human QAs by clicking a URL, never by running anything. |
| Database | **SQLite (node:sqlite, built-in)** — file-backed for dev/staging/prod; `:memory:` for ephemeral test isolation; no external service required; optional PostgreSQL backend for scale-out | Keeps destructive experimentation off production. Ephemeral `:memory:` instances isolate e2e runs from the shared dev database (§7.1). |
| E2E verification | **vitest + compiled-dist smoke tests** — the full ~2060-test vitest suite runs against the built distribution in CI, covering every route and wire contract end-to-end | Anthropic's harness research found agents skip end-to-end verification unless given browser tooling and told to use it; this closed the biggest quality gap in their long-running trials. |
...
- **Allow:** `npm` scripts, `node` (≥22.13), `tsc`, `biome`, `vitest`, `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide. (No external DB CLI — SQLite is built-in.)
